...
const Home = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:6000/get')
            .then(result => setTodos(result.data))
            .catch(err => console.log(err));
    }, []);

    const edit = (id) => {
        axios.put(`http://localhost:6000/edit/${id}`)
            .then(result => {
    ...
    const Update = (id, updatedTask) => {
        axios.put(`http://localhost:6000/update/${id}`, { task: updatedTask })
            .then(result => {
                console.log(result.data);
                const updatedTodos = todos.map(todo => {
                    if (todo._id === id) {
    ...
    const Hdelete = (id) => {
        axios.delete(`http://localhost:6000/delete/${id}`)
            .then(result => {
                console.log(result.data);
